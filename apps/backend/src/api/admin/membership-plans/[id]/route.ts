...
export const POST = async (req: MedusaRequest<AdminUpdateMembershipPlanType>, res: MedusaResponse) => {
  ...
  const stripeIds = priceChanged || variantChanged
    ? await (async () => {
        ...
        const stripePrice = await stripe.prices.create({ product: stripeProduct.id, currency: price.currency_code, unit_amount: stripeUnitAmount(price.amount, price.currency_code), recurring: { interval: input.billing_period === "monthly" ? "month" : "year" } });
